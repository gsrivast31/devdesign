...
Template[getTemplate('post_edit')].events({
  ...
  'click input[type=submit]': function(e, instance){
    ...
    if (properties) {
      ...
      var oldCategories = Posts.findOne({_id:post._id}).categories;
      var newCategories = properties.categories;

      var oldCategoryIds = _.map(oldCategories, function(category){
        return category._id;
      });

      var newCategoryIds = _.map(newCategories, function(category){
        return category._id;
      });

      var removedCategories = _.difference(oldCategoryIds, newCategoryIds);
      var addedCategories = _.difference(newCategoryIds, oldCategoryIds);
      ...
      Posts.update(post._id, updateObject, function(error){
        if(error){
          ...
          /* GAURAV */
          Meteor.call('updateCategoryCounts', addedCategories, removedCategories, function(error) {
            if (error) {
    ...
    if(confirm("Are you sure?")){
      ...
      Meteor.call("deletePostById", post._id, function(error) {
        if (error) {
          ...
          /* GAURAV */
          var addedCategories = [];
          var removedCategories = _.map(post.categories, function(category){
            return category._id;
          });
          Meteor.call('updateCategoryCounts', addedCategories, removedCategories, function(error) {
            if (error) {
