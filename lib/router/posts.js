...
PostsListController = FastRender.RouteController.extend({
  ...
  onAfterAction: function() {
    ...
    if (this.params._id) {
      ...
      if (Session.get('topCategoryFirstChildDeveloper') === '' || 
        Session.get('topCategoryFirstChildDesigner') === '' || 
        Session.get('topCategoryFirstChildStartup') === '') {
        
        var category = Categories.findOne({_id: this.params._id});
        var parent_category_id = category.parent;

        var developerId = Session.get('developerId');
        var designerId = Session.get('designerId');
        var startupId = Session.get('startupId');
  
        var current_top_category_id = Session.get('topCategory');

        while (parent_category_id && parent_category_id !== developerId 
          && parent_category_id !== designerId 
          && parent_category_id !== startupId) {
          category = Categories.findOne({_id : parent_category_id});
          parent_category_id = category.parent;
        }

        Session.set('topCategory', parent_category_id);

        if (parent_category_id && parent_category_id === developerId) {
          Session.set('topCategoryFirstChildDeveloper', category._id);
        } else if (parent_category_id && parent_category_id === designerId) {
          Session.set('topCategoryFirstChildDesigner', category._id);
        } else if (parent_category_id && parent_category_id === startupId) {
          Session.set('topCategoryFirstChildStartup', category._id);
        }
      }
    }
  }
